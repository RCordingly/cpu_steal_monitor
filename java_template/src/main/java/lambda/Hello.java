...
public class Hello implements RequestHandler<Request, HashMap<String, Object>> {
    ...
    public HashMap<String, Object> handleRequest(Request request, Context context) {
        ...
        inspector.addAttribute("Monitor State", request.getMode());
        
        if (request.getMode() == 1) {
            
            ArrayList<Long> stealList = new ArrayList<>();
            
            for (int i = 0; i < 600; i++) {
                
                Long steal = (Long)  inspector.getAttribute("vmcpusteal");
                stealList.add(steal);
                inspector.inspectContainer();
            ...
            inspector.addAttribute("message", "Used monitor mode...");
            inspector.addAttribute("StealList", stealList);
            ...
            inspector.addAttribute("message", "Used calc mode...");
            return inspector.finish();
        }
    }
